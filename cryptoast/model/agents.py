...
class Backtest:
    ...
    def _step(
        ...
        periodic[timestamp]["value"] = (
            periodic[timestamp]["cash"]
            + periodic[timestamp]["assets"] * kline[timestamp]["close"]
        )
    ...
    def _fill_order(
        ...
        if order["action"] == "buy":
            ...
            price = fkline["close"] + (
                1 * slippage_pct * abs(slippage_reference - fkline["close"])
            )
            precision = get_precision(stepsize)
            max_size = prec_floor(
                cash / (price * (1 + commission)), precision=precision
            )
    ...
    def from_kline(self, kline, start_index=0):
        ...
        self.position = {
            ...
            "timestamp": self.timestamps[self.start_index],
        }
        ...
        self._orders = dict()
        self._periodic = dict()
        self._periodic[self.position["timestamp"]] = {
            "cash": self.init_cash,
            "assets": self.init_assets,
            "value": (
                self.init_cash
                + self.init_assets * self.kline[self.position["timestamp"]]["close"]
            ),
        }
        self._history = {
            0: [[0, 0, 0] for _ in range(self.memory - 1)]
    ...
    def step(self, order, verbose=0):
        ...
        Backtest._step(
            ...
            slippage_steps=self.slippage_steps,
...
class SingleAssetEnv(gym.Env):
    ...
    def __init__(
        ...
        self.indicator_cols = klmngr[klmngr.assets[0]].indicators.columns.to_series()
        self.signal_cols = klmngr[klmngr.assets[0]].signals.columns.to_series()
        self.rel_indicators_mask = self.indicator_cols.str.startswith(
            self._rel_indicators_stem
        )
        self.abs_indicators_mask = self.indicator_cols.str.startswith(
            self._abs_indicators_stem
        )
        self.n_obs_cols = len(
            self.signal_cols
        )  # self.rel_indicators_mask.sum() + self.abs_indicators_mask.sum()
    ...
    def get_observation(self, timestamp, window):
        ...
        transactions_history = np.hstack(
            [0, 0]
            + [
    ...
    def get_observation_old(self, timestamp, window):
        ...
        # timestamp = timestamp + pd.Timedelta(days=1)
        close_values = (
            self.kline.loc[:timestamp, :].close.values[-window:].reshape(-1, 1)
        )
        # indicator_values = self.kline.indicators.loc[:timestamp, :].fillna(-1).values[-window:]
        periodic_values = np.array(self.backtest.history)[:, :2]
        observation_raw = np.hstack(
            (close_values,)
        )  # indicator_values # periodic_values
        if self.current_step % 720 == 0:  # 720 == 24 * 30 # self.current_step == 0:
            ...
            scaler_history = 5000
            hist_close_values = (
                self.kline.loc[:timestamp, :]
                .close.values[-scaler_history:]
                .reshape(-1, 1)
            )
            """
        ...
        observation = np.hstack(
            [
                self.scaler.fit_transform(observation_raw[:, [_]])
                for _ in range(self.indicator_cols)
            ]
        )
    ...
    def get_reward(self):
        ...
        while True:
            try:
                value_t0 = getattr(self.backtest, "_periodic")[previous_timestamp][
                    "value"
                ]
    ...
    def reset(self):
        ...
        while True:
            ...
            index_to_drop = kline.index[
                (kline.index < self.datetimes[0]) | (kline.index >= self.datetimes[1])
            ]
    ...
    def step(self, action):
        ...
        reward = self.get_reward()
            ...
            timestamp == self.kline.index.max() - timedelta(hours=1)
            or (
                not (self.allow_gaps)
                and any(
                    [
                        timestamp + timedelta(hours=1) not in self.kline.index,
                        timestamp + timedelta(hours=2) not in self.kline.index,
                    ]
                )
            )
            or (self.current_step == self.episode_steps)
        )
        info = self.get_info()
